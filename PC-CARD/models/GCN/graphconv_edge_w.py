...
class GraphConvEdgeWeight(nn.Module):
    ...
    def __init__(self,
                 ...
                 bias=True,
                 activation=None):
        super(GraphConvEdgeWeight, self).__init__()
    ...
    def forward(self, graph, feat, weight=None):
        ...
        * Weight shape: "math:`(\text{in_feats}, \text{out_feats})`.
        ...
        if self._norm == 'both':
            ...
            src, dst, e_ids = graph.edges(form='all')
            degs = th.zeros(len(graph.nodes())).to(feat.device)
            for e_id in e_ids:
                src_node = src[e_id]
                degs[src_node] = degs[src_node] + e_weights[e_id]
